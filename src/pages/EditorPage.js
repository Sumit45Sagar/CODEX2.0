...
const EditorPage = () => {
    ...
    const [clients, setClients] = useState([]);
    const [output, setOutput] = useState('');
    const language = location.state?.language || 'javascript'; // Get the language from location state
    ...
    const runCode = async () => {
        ...
        try {
            const response = await executeCode(language, code); // Use the selected language
            setOutput(response.run.output); 
        } catch (error) {
            toast.error('Error running the code');
            console.error(error);
